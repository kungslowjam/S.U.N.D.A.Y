...
All notable changes to SUNDAY are documented in this file.
...
- **agentskills.io spec adoption** — canonical `SKILL.md` format with YAML frontmatter following the [agentskills.io](https://agentskills.io/specification) open standard.
  ...
  - Sourced subdirectory layout (`~/.sunday/skills/<source>/<name>/`)
...
- **Skills learning loop** — trace tagging, pattern discovery, DSPy/GEPA optimization.
  ...
  - `SkillOverlay` — sidecar storage at `~/.sunday/learning/skills/<name>/optimized.toml`
...
- **CLI commands:**
  - `sunday skill list` / `info` / `run` / `install` / `sync` / `sources` / `update` / `remove` / `search`
  - `sunday skill discover` — mine traces for recurring tool patterns
  - `sunday skill show-overlay` — inspect optimization output
  - `sunday optimize skills` — run DSPy/GEPA per-skill optimization
  - `sunday bench skills` — run the PinchBench skills benchmark
...
- **OpenClaw category/name queries** — `sunday skill install openclaw:owner/slug` now correctly splits into category + name match
