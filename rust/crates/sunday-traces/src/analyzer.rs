...
impl<'a> TraceAnalyzer<'a> {
    ...
    pub fn overall_stats(&self) -> Result<TraceStats, SUNDAYError> {
    ...
    pub fn stats_by_agent(&self) -> Result<HashMap<String, TraceStats>, SUNDAYError> {
        let traces = self.store.list_traces(10000, 0)?;
    ...
    pub fn stats_by_model(&self) -> Result<HashMap<String, TraceStats>, SUNDAYError> {
        let traces = self.store.list_traces(10000, 0)?;
