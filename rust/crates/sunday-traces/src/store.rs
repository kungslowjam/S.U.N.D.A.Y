...
use sunday_core::{SUNDAYError, Trace};
...
impl TraceStore {
    pub fn new(db_path: &Path) -> Result<Self, SUNDAYError> {
    ...
    pub fn in_memory() -> Result<Self, SUNDAYError> {
        Self::new(Path::new(":memory:"))
    }

    pub fn save(&self, trace: &Trace) -> Result<(), SUNDAYError> {
    ...
    pub fn get(&self, trace_id: &str) -> Result<Option<Trace>, SUNDAYError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
    ...
    pub fn count(&self) -> Result<usize, SUNDAYError> {
        let conn = self.conn.lock();
        let count: i64 = conn
