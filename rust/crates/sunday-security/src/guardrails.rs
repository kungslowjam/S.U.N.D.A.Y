...
use sunday_core::error::SUNDAYError;
use sunday_core::{EventBus, EventType, GenerateResult, Message};
use sunday_engine::traits::{InferenceEngine, TokenStream};
        ...
        match self.mode {
            ...
            RedactionMode::Block => {
                ...
                Err(SUNDAYError::Security(
                    sunday_core::error::SecurityError::Blocked(format!(
...
mod tests {
    ...
    fn test_guardrails_block_mode() {
        ...
        let err = guardrails
            .generate(&[Message::user("Hi")], "mock", 0.7, 100, None)
            .unwrap_err();
        assert!(matches!(err, SUNDAYError::Security(_)));
    }
}
