...
impl InferenceEngine for LlamaCppEngine {
    ...
    fn generate(
        ...
        let resp = self
            ...
            .send()
            .map_err(|e| {
                SUNDAYError::Engine(EngineError::Connection(format!(
        ...
        if !resp.status().is_success() {
            ...
            return Err(SUNDAYError::Engine(EngineError::Http(format!(
                "llama.cpp returned {}: {}",
                status, body
            ))));
        }
        ...
        let data: Value = resp.json().map_err(|e| {
            SUNDAYError::Engine(EngineError::Deserialization(e.to_string()))
        })?;
    ...
    async fn stream(
        ...
        let async_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                SUNDAYError::Engine(EngineError::Connection(e.to_string()))
            })?;
    ...
    fn list_models(&self) -> Result<Vec<String>, SUNDAYError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|_| {
                SUNDAYError::Engine(EngineError::Connection(
