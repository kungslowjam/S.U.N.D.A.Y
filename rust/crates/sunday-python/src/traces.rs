...
pub struct PyTraceStore {
    pub inner: Arc<sunday_traces::TraceStore>,
}
...
impl PyTraceStore {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        let inner = match path {
            Some(p) => sunday_traces::TraceStore::new(std::path::Path::new(p)),
            None => sunday_traces::TraceStore::in_memory(),
        }
...
pub struct PyTraceCollector {
    inner: sunday_traces::TraceCollector,
}
...
impl PyTraceCollector {
    ...
    fn new(store: &PyTraceStore) -> Self {
        Self {
            inner: sunday_traces::TraceCollector::new(Arc::clone(&store.inner)),
        }
    }
...
pub struct PyTraceAnalyzer {
    store: Arc<sunday_traces::TraceStore>,
}
...
impl PyTraceAnalyzer {
    ...
    fn stats(&self) -> PyResult<String> {
        let analyzer = sunday_traces::TraceAnalyzer::new(&self.store);
        let stats = analyzer
            .overall_stats()
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
