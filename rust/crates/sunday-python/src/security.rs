...
pub struct PySecretScanner {
    inner: sunday_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: sunday_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: sunday_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: sunday_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: sunday_security::GuardrailsEngine<sunday_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => sunday_security::RedactionMode::Redact,
            "block" => sunday_security::RedactionMode::Block,
            _ => sunday_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: sunday_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<sunday_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = sunday_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: sunday_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: sunday_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: sunday_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: sunday_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: sunday_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: sunday_security::RateLimiter::new(
                sunday_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
pub struct PyTaintSet {
    inner: sunday_security::TaintSet,
}
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: sunday_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => sunday_security::TaintLabel::Pii,
            "secret" => sunday_security::TaintLabel::Secret,
            "user_private" => sunday_security::TaintLabel::UserPrivate,
            "external" => sunday_security::TaintLabel::External,
            _ => sunday_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &sunday_security::TaintSet::from_labels(&[taint_label]),
        );
    }
