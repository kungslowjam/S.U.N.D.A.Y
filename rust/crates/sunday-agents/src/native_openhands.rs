...
use sunday_core::{AgentContext, AgentResult, SUNDAYError, ToolResult};
    ...
    async fn run(
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        sunday_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        sunday_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    SUNDAYError::Agent(sunday_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<sunday_engine::Engine>>;
