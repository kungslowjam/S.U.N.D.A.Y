...
This tutorial demonstrates how to use SUNDAY to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Jarvis` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `sunday scheduler` CLI
...
Register each script as a recurring task with `sunday scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | sunday channel send slack
...
See `sunday channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let SUNDAY auto-select from what is available.
