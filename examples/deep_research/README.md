...
using SUNDAY. The assistant uses an orchestrator agent loop with web
...
- SUNDAY installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `sunday eval`
runner.
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `sunday agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `sunday.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/sunday/recipes/) -- composable TOML configs that
