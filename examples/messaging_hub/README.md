...
SUNDAY orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting SUNDAY to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- SUNDAY installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the SUNDAY channel docs).
...
SUNDAY supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/sunday/config.toml` for the full list of channel and agent
options.
...
this via the SUNDAY scheduler:

```bash
sunday scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/sunday/recipes/data/operators/` for examples.
