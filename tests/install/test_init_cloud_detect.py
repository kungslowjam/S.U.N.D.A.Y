"""Tests for init's cloud auto-detect and from-bare-sunday flag."""
...
def test_init_accepts_from_bare_sunday_flag(
    tmp_sunday_home: Path, monkeypatch
) -> None:
    """The --from-bare-sunday flag exists and suppresses the launch-chat prompt."""
    ...
    result = runner.invoke(
        init,
        ["--from-bare-sunday", "--engine", "ollama", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
    # When --from-bare-sunday, we expect NOT to see a "launch chat" affordance.
...
def test_init_from_bare_sunday_skips_engine_prompt(
    tmp_sunday_home: Path, monkeypatch
) -> None:
    """--from-bare-sunday must not hang on the engine-selection prompt
    ...
    result = runner.invoke(
        init,
        ["--from-bare-sunday", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
