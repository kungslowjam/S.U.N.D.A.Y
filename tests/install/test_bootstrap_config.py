"""Tests for sunday.cli._bootstrap.write_initial_config."""
...
from sunday.cli import _bootstrap
from sunday.core.config import GpuInfo, HardwareInfo


def test_writes_minimal_local_config(tmp_sunday_home: Path) -> None:
    hw = HardwareInfo(
        platform="linux",
        cpu_brand="AMD EPYC",
        cpu_count=16,
    ...
    cfg_path = tmp_sunday_home / "config.toml"
...
def test_writes_cloud_config(tmp_sunday_home: Path) -> None:
    ...
    data = tomllib.loads((tmp_sunday_home / "config.toml").read_text())
    ...
    assert "sk-ant-test" not in (tmp_sunday_home / "config.toml").read_text()


def test_includes_install_provenance(tmp_sunday_home: Path, monkeypatch) -> None:
...
def test_writes_seed_files_if_absent(tmp_sunday_home: Path) -> None:
    hw = HardwareInfo(platform="linux", cpu_brand="x", cpu_count=1, ram_gb=4.0)
    _bootstrap.write_initial_config(hardware=hw, engine="ollama", model="qwen3.5:2b")
    assert (tmp_sunday_home / "SOUL.md").exists()
    assert (tmp_sunday_home / "MEMORY.md").exists()
    assert (tmp_sunday_home / "USER.md").exists()
    assert (tmp_sunday_home / "skills").is_dir()


def test_does_not_overwrite_existing_seeds(tmp_sunday_home: Path) -> None:
    soul = tmp_sunday_home / "SOUL.md"
...
def test_overwrites_existing_config_toml(tmp_sunday_home: Path) -> None:
    cfg = tmp_sunday_home / "config.toml"
...
def test_handles_special_chars_in_model_name(tmp_sunday_home: Path) -> None:
...
def test_sunday_config_has_install_provenance_fields() -> None:
