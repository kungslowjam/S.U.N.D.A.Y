"""Tests for sunday.evals.backends.external.hermes_agent.HermesBackend."""
...
from sunday.evals.backends.external._subprocess_runner import SubprocessResult
    ...
    return ThirdPartyConfig(
        entries={
            "hermes": ThirdPartyEntry(
                ...
                runner_script=(
                    "src/sunday/evals/backends/external/_runners/hermes_runner.py"
                ),
                python_executable="",
            )
        }
...
class TestHermesBackend:
    def test_generate_full_builds_correct_subprocess_command(
        ...
        with (
            patch(
                "sunday.evals.backends.external.hermes_agent.load_third_party_config",
                return_value=cfg,
            ),
            patch(
                "sunday.evals.backends.external.hermes_agent.verify_commit_pin",
            ),
            patch(
                "sunday.evals.backends.external.hermes_agent.run_one_shot",
            ) as mock_run,
        ):
