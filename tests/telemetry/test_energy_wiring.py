...
from sunday.telemetry.aggregator import AggregatedStats, TelemetryAggregator
...
_bench_mod = importlib.import_module("sunday.cli.bench_cmd")
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "sunday.telemetry.energy_monitor.create_energy_monitor",
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "sunday.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "sunday.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "sunday.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "sunday.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path
