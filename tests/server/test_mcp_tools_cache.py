...
def test_returns_tools_from_mcp_server(mock_load_config: MagicMock):
    ...
    with (
        patch("sunday.mcp.transport.StreamableHTTPTransport"),
        patch("sunday.mcp.client.MCPClient"),
        patch("sunday.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
