"""Tests for ``sunday connect`` CLI command."""
...
def test_connect_list_no_connectors() -> None:
    ...
    runner = CliRunner()
    with mock.patch(
        "sunday.cli.connect_cmd.connect.__wrapped__"
        if hasattr(cli, "__wrapped__")
        else "sunday.core.registry.ConnectorRegistry.items",
        return_value=(),
    ):
        with mock.patch(
            "sunday.core.registry.ConnectorRegistry.items",
            return_value=(),
        ):
...
def test_connect_specific_source(tmp_path: object) -> None:
    ...
    with (
        mock.patch(
            "sunday.core.registry.ConnectorRegistry.contains",
            return_value=True,
        ),
        mock.patch(
            "sunday.core.registry.ConnectorRegistry.get",
