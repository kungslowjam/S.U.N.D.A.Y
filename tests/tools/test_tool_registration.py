"""Verify that importing sunday.tools registers all built-in tools."""
...
def _reload_tool_modules() -> None:
    """Reload all sunday.tools.* submodules to re-trigger @register decorators.

    The autouse ``_clean_registries`` fixture clears all registries before each
    test.  A plain ``import sunday.tools`` won't re-register because the
