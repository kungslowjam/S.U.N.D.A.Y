...
# Spotify
sunday connect spotify

# Strava
sunday connect strava
```
...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.sunday/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.sunday/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.sunday/connectors/news_rss.json << 'EOF'
{"feeds": [
...
```bash
CARTESIA_API_KEY="sk_car_..." sunday digest --fresh
```
...
```bash
sunday digest --fresh          # Generate a new digest now
sunday digest                  # Show today's cached digest
sunday digest --text-only      # Print text without audio
sunday digest --history        # Show past digests
sunday digest --schedule "0 6 * * *"   # Set daily schedule
sunday digest --schedule off   # Disable schedule
sunday digest --schedule       # Show current schedule
```
...
The `persona` field loads a prompt file from `configs/sunday/prompts/personas/{name}.md`. The default `sunday` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.sunday/connectors/news_rss.json`:
...
**"No digest for today"** — Run `sunday digest --fresh` to generate one.

**Empty sections** — Check connector status with `sunday connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
