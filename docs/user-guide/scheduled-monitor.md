...
This writes a pre-configured `~/.sunday/config.toml` for the operative agent with scheduling support.
...
sunday scheduler status <task-id>

# Pause / resume / delete a task
sunday scheduler pause <task-id>
sunday scheduler resume <task-id>
sunday scheduler delete <task-id>

# Run a task immediately (outside its schedule)
sunday scheduler run <task-id>

# Stop the scheduler daemon
sunday scheduler stop
```
...
- **All data stays local**: State is stored in `~/.sunday/` using the configured memory backend. Nothing leaves your machine.

## Troubleshooting

**"Scheduler not running"** -- Start the scheduler daemon with `sunday scheduler start`. It must be running for scheduled tasks to execute.

**Task doesn't run on time** -- Check that Ollama is running (`ollama serve`). The scheduler triggers the agent, but the agent needs an inference engine. Verify the schedule with `sunday scheduler status <task-id>`.
...
**Memory grows too large** -- Periodically review with `sunday memory stats`. Clear old entries with `sunday memory clear --before 2026-01-01` if needed.
