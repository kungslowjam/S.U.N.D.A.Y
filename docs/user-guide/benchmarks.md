...
SUNDAY ships with two benchmarks:
...
sunday bench run -n 50

# Run only the latency benchmark
sunday bench run -b latency

# Run only the throughput benchmark with 20 samples
sunday bench run -b throughput -n 20

# Specify model and engine
sunday bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
sunday bench run --json

# Write JSONL results to a file
sunday bench run -o results.jsonl

# Combine options
sunday bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from sunday.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
sunday bench run -b context_length
```
