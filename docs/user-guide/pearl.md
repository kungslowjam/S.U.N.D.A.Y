...
SUNDAY includes a thin `sunday pearl` wrapper for Pearl's native command
...
`sunday pearl` looks for `pearld`, `oyster`, and `prlctl` on `PATH`, then under
`$PEARL_HOME/bin`.
...
```bash
sunday pearl node -- --help
sunday pearl wallet -- --help
sunday pearl ctl -- --help
```

These map directly to:

| SUNDAY command | Pearl binary |
|---|---|
| `sunday pearl node` | `pearld` |
| `sunday pearl wallet` | `oyster` |
| `sunday pearl ctl` | `prlctl` |

The command format is always `sunday pearl <command>`. Pearl-native arguments
...
`sunday mine` is the SUNDAY mining lifecycle. `sunday pearl` is an escape
