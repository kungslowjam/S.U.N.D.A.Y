...
SUNDAY provides a command-line interface through the `sunday` command. Built on [Click](https://click.palletsprojects.com/), it offers subcommands for querying models, managing memory, running benchmarks, and serving an OpenAI-compatible API.

## Global Options

```bash
sunday --version   # Print the SUNDAY version
sunday --help      # Show top-level help with all subcommands
```

## `sunday init`

Detect local hardware (CPU, GPU, RAM) and generate a configuration file at `~/.sunday/config.toml`.

```bash
sunday init           # Interactive — refuses to overwrite existing config
sunday init --force   # Overwrite existing config without prompting
```
...
```bash
sunday ask "Explain quantum computing"
```

**Agent mode** routes the query through an agent that can use tools and manage multi-turn interactions:

```bash
sunday ask --agent orchestrator "What is 2+2?"
sunday ask --agent orchestrator --tools calculator,think "Calculate sqrt(144) + 3^2"
sunday ask --agent simple "Hello"
```
...
```bash
# Basic query
sunday ask "What is machine learning?"

# Specify a model
sunday ask -m qwen3:8b "Summarize this concept"

# Use the orchestrator agent with tools
sunday ask --agent orchestrator --tools calculator "What is 15% of 340?"

# Get JSON output
sunday ask --json "Hello"

# Disable memory context injection
sunday ask --no-context "Tell me about Python"

# Set maximum token generation
sunday ask --max-tokens 2048 "Write a detailed essay about AI"
```
...
Access Pearl's native node, wallet, and RPC tools from the SUNDAY CLI.

```bash
sunday pearl doctor
sunday pearl node -- <pearld args>
sunday pearl wallet -- <oyster args>
sunday pearl ctl -- <prlctl args>
sunday pearl address
```

All Pearl wrapper commands use the `sunday pearl <command>` shape. The
pass-through commands map to Pearl's native binaries:

| SUNDAY command | Pearl binary | Use |
|--------------------|--------------|-----|
| `sunday pearl doctor` | n/a | Check whether `pearld`, `oyster`, and `prlctl` are discoverable |
| `sunday pearl node` | `pearld` | Run the Pearl full node |
| `sunday pearl wallet` | `oyster` | Run the Oyster wallet daemon |
| `sunday pearl ctl` | `prlctl` | Query Pearl node or wallet RPC |
| `sunday pearl address` | `prlctl --wallet getnewaddress` | Generate a wallet address from Oyster |
...
### `sunday memory index <path>`

Index documents from a file or directory into the memory store.

```bash
sunday memory index ./docs/
sunday memory index ./notes.md
sunday memory index ./data/ --chunk-size 256 --chunk-overlap 32
sunday memory index ./docs/ --backend sqlite
```
...
### `sunday memory search <query>`

Search the memory store for relevant document chunks.

```bash
sunday memory search "machine learning basics"
sunday memory search -k 10 "neural networks"
sunday memory search --backend faiss "embeddings"
```
...
```bash
sunday memory stats
sunday memory stats --backend sqlite
```
...
```bash
sunday telemetry stats
sunday telemetry stats -n 5    # Show top 5 models
```
...
### `sunday telemetry export`

Export raw telemetry records in JSON or CSV format.

```bash
sunday telemetry export                          # JSON to stdout
sunday telemetry export --format csv             # CSV to stdout
sunday telemetry export --format json -o data.json  # JSON to file
sunday telemetry export -f csv -o metrics.csv    # CSV to file
```
...
### `sunday telemetry clear`

Delete all telemetry records from the database.

```bash
sunday telemetry clear         # Interactive confirmation
sunday telemetry clear --yes   # Skip confirmation
```
...
```bash
sunday bench run                               # Run all benchmarks, 10 samples
sunday bench run -n 20                         # 20 samples per benchmark
sunday bench run -b latency                    # Only the latency benchmark
sunday bench run -b throughput -n 50 --json    # Throughput, 50 samples, JSON output
sunday bench run -o results.jsonl              # Write JSONL results to file
sunday bench run -m qwen3:8b -e ollama         # Specific model and engine
```
...
```bash
sunday channel send slack "Hello from Jarvis!"
sunday channel send discord "Build complete"
```
...
```bash
sunday serve                                 # Default host/port from config
sunday serve --port 8000                     # Custom port
sunday serve --host 0.0.0.0 --port 9000      # Bind to all interfaces
sunday serve --model qwen3:8b                # Specify default model
sunday serve --agent orchestrator            # Route requests through an agent
```
...
```bash
sunday learning run
sunday learning run --autonomy auto    # auto-apply all edits
sunday learning run --autonomy manual  # dry-run, everything goes to review
```
...
```bash
sunday learning history
sunday learning history --limit 5
```

### `sunday learning show`
...
### `sunday learning reject`

Reject a pending edit.

```bash
sunday learning reject <edit-id>
sunday learning reject <edit-id> --reason "too aggressive"
```

### `sunday learning rollback`
...
### `sunday learning benchmark`

Personal benchmark management.

```bash
sunday learning benchmark show       # current stats
sunday learning benchmark refresh    # manual refresh
```

### `sunday learning daemon`

Background learning daemon.

```bash
sunday learning daemon start
sunday learning daemon stop
sunday learning daemon status
```
