...
from sunday.tools._stubs import ToolSpec
...
# Execute a tool call
from sunday.core.types import ToolCall
...
```python
from sunday.tools.retrieval import RetrievalTool
from sunday.memory.sqlite import SQLiteMemory
...
```python
from sunday.tools.llm_tool import LLMTool
...
```python
from sunday.scheduler.tools import ScheduleTaskTool
from sunday.scheduler.scheduler import TaskScheduler
from sunday.scheduler.store import SchedulerStore

store = SchedulerStore(db_path="~/.sunday/scheduler.db")
scheduler = TaskScheduler(store=store, system=sunday_system)
scheduler.start()
...
```bash
sunday ask --agent orchestrator --tools my_tool "Process this data"
```
...
# Multiple tools
sunday ask --agent orchestrator --tools calculator,think "Solve: 2x + 5 = 13"

# All available tools (list them)
sunday ask --agent orchestrator --tools calculator,think,retrieval,file_read "..."
```
