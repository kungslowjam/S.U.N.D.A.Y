...
!!! note "Optional component"
    The scheduler is a standalone module (`sunday.scheduler`). It is not wired into the default `Jarvis` / `JarvisSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `sunday scheduler start`.
...
The `sunday scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
sunday scheduler list --status active

# Paused tasks
sunday scheduler list --status paused
```
...
sunday scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
sunday scheduler resume a3f9b12c4d8e
```
...
sunday scheduler cancel a3f9b12c4d8e
```
...
sunday scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.sunday/scheduler.db")  # (1)!
...
4. Starts a daemon thread named `"sunday-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.sunday/config.toml`.

```toml title="~/.sunday/config.toml"
[scheduler]
enabled = false
db_path = "~/.sunday/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.sunday/scheduler.db`     | SQLite database path                       |
