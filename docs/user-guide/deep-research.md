...
This writes a pre-configured `~/.sunday/config.toml` for the deep research agent.
...
SUNDAY chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
sunday ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
sunday ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"

# Index more documents
sunday memory index ~/Downloads/reports/
sunday memory index ./notes.md

# Search memory directly
sunday memory search "project timeline"
sunday memory search -k 20 "budget estimates"
...
sunday ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
sunday ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
sunday ask "What meetings did I have with Alice last month?"

# Extract structured data
sunday ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
sunday ask "Compare our internal benchmarks with the latest published results"
```
...
# Single file
sunday memory index ./report.pdf

# Custom chunk size for long documents
sunday memory index ./paper.pdf --chunk-size 1024 --chunk-overlap 128
```
...
```bash
sunday memory index ~/Papers/*.pdf
```
...
```bash
curl -s https://example.com/article | sunday memory index --stdin --source "example.com"
```

### Code repositories

```bash
sunday memory index ./src/ --chunk-size 256
```
...
**"No results found"** -- Make sure you have indexed documents first with `sunday memory index`. Check indexed content with `sunday memory stats`.
