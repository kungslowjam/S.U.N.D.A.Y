...
SUNDAY has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.sunday/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.sunday/telemetry.db")
...
sunday telemetry stats -n 5          # Top 5 models only

# Export records
sunday telemetry export              # JSON to stdout
sunday telemetry export -f csv       # CSV to stdout
sunday telemetry export -o data.json # JSON to file
sunday telemetry export -f csv -o metrics.csv

# Clear all records
sunday telemetry clear               # With confirmation prompt
sunday telemetry clear --yes         # Without confirmation
```
