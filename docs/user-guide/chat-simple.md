...
A lightweight conversational AI with no tools and no agent overhead. This is the simplest possible SUNDAY setup: just Ollama and a local model. Ideal for general-purpose chat, Q&A, brainstorming, and getting started quickly.
...
### 2. Install and initialize SUNDAY

```bash
git clone https://github.com/open-sunday/SUNDAY.git
cd SUNDAY
uv sync
sunday init --preset chat-simple
```

### 3. Ask a question

```bash
sunday ask "What is quantum computing?"
```
...
```bash
# Single question
sunday ask "Explain the difference between TCP and UDP"
...
sunday ask -m qwen3.5:9b "Explain general relativity"

# Adjust temperature (0.0 = deterministic, 1.0 = creative)
sunday ask -t 0.2 "List the planets in our solar system"

# Output raw JSON
sunday ask --json "What is 2+2?"
```
...
To switch models, either edit `~/.sunday/config.toml` or override per-query:

```bash
sunday ask -m qwen3.5:35b "Write a detailed comparison of REST and GraphQL"
```
...
1. Start the backend: `sunday serve` (or `./scripts/quickstart.sh`)
2. Download and open the desktop app from the [releases page](https://github.com/open-sunday/SUNDAY/releases)
...
${EDITOR:-nano} ~/.sunday/config.toml
...
sunday ask -m deepseek-r1:14b "Hello"
```

**Use an environment variable:**

```bash
OPENSUNDAY_MODEL=qwen3.5:9b sunday ask "Hello"
```
...
**Browser app not loading** -- Make sure both the backend (`sunday serve`) and frontend are running. The `./scripts/quickstart.sh` script starts both automatically.
