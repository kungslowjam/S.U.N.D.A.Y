...
SUNDAY has two types of integrations:
...
```json
{
    "display_information": { "name": "SUNDAY" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "SUNDAY", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
- You DM @SUNDAY in Slack → Socket Mode receives the event in real-time
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "SUNDAY" |
...
3. **Connect in SUNDAY:**
   ...
   - CLI: `uv run sunday connect gmail_imap`
...
Slack serves two purposes in SUNDAY:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [SUNDAY icon](https://github.com/open-sunday/SUNDAY/blob/main/assets/sunday-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **SUNDAY** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "SUNDAY"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @SUNDAY` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @SUNDAY` in the channel |
...
3. **Connect in SUNDAY:**
   ...
   - CLI: `uv run sunday connect notion`
...
3. **Connect in SUNDAY:**
   ...
   - CLI: `uv run sunday connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the SUNDAY desktop app
...
3. **Connect in SUNDAY:**
   ...
   - CLI: `uv run sunday connect outlook`
...
2. **Connect in SUNDAY:**
   ...
   - CLI: `uv run sunday connect obsidian --path /path/to/vault`
...
4. **Connect in SUNDAY:**
   ...
   - CLI: `uv run sunday connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run sunday deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.sunday/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.sunday/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
