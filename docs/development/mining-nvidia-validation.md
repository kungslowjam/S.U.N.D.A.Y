...
prove SUNDAY wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through SUNDAY.
...
- Docker container `sunday-pearl-miner` starts.
- `~/.sunday/runtime/mining.json` is written.
...
```bash
docker ps --filter name=sunday-pearl-miner
cat ~/.sunday/runtime/mining.json
uv run sunday mine logs --tail 200
...
## Verify SUNDAY Inference Uses Mining Endpoint

Run:

```bash
uv run sunday mine doctor
uv run sunday ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/sunday/mining/_metrics.py`.
...
```bash
uv run sunday mine stop
docker ps --filter name=sunday-pearl-miner
test ! -e ~/.sunday/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `sunday ask` no longer routes through `vllm-pearl-mining` unless another
...
- SUNDAY inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run sunday mine doctor
uv run sunday mine status || true
uv run sunday mine logs --tail 300 || true
docker inspect sunday-pearl-miner || true
