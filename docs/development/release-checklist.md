...
2. **`sunday` (no args)** drops into a chat session within 2 s.
...
6. **`sunday doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`sunday-uninstall`** removes `~/.sunday/` and `~/.local/bin/sunday*`. Verify with `ls`.
...
- Delete `~/.sunday/config.toml` — verify bare `sunday` re-runs init.
- Delete `~/.sunday/.venv` — verify re-running curl heals it.
