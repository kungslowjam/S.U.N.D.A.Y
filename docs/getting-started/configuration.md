---
title: Configuration
description: Complete reference for SUNDAY configuration
---

# Configuration

SUNDAY uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
SUNDAY creates the `~/.sunday/` directory and populates it with a default config when you run `sunday init`.
...
3. Writes `~/.sunday/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `sunday init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, SUNDAY automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, SUNDAY uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
sunday ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.sunday/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `sunday serve`.
...
```bash
sunday serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
| `db_path` | string | `~/.sunday/telemetry.db` | Path to the SQLite telemetry database. |
...
| `db_path` | string | `~/.sunday/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.sunday/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.sunday/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `sunday init`, SUNDAY probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.sunday/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure SUNDAY entirely from Python without a TOML file:
...
SUNDAY respects the following environment variables:
