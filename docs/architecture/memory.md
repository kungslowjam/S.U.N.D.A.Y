...
- **Persistence:** Data persists across restarts in `~/.sunday/memory.db`
...
from sunday.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from sunday.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `sunday.tools.storage.*`.
...
```python
from sunday.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from sunday.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from sunday.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.sunday/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
