...
!!! note "Backward compatibility"
    The canonical locations are now `sunday.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `sunday.core.types` (for `RoutingContext`). The old `sunday.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `sunday.learning._stubs`.
...
```bash
sunday ask --router learned "What is the capital of France?"
```
...
```python
from sunday.learning.router import HeuristicRouter, build_routing_context
...
```python
from sunday.learning.trace_policy import TraceDrivenPolicy
from sunday.traces.analyzer import TraceAnalyzer
...
```python
from sunday.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from sunday.learning.sft_policy import SFTPolicy
```
...
```python
from sunday.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from sunday.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from sunday.learning.heuristic_reward import HeuristicRewardFunction
...
store = TraceStore("~/.sunday/traces.db")
...
over SUNDAY's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `sunday-learning`
