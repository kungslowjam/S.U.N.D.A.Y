...
The Engine primitive provides the **inference runtime** -- the layer that connects SUNDAY to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
SUNDAY automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from sunday.engine import discover_engines, discover_models
...
Engine hosts and defaults are configured in `~/.sunday/config.toml` using **nested per-engine sub-sections**:
...
```python
from sunday.engine._base import messages_to_dicts
...
```python
from sunday.engine import EngineConnectionError
