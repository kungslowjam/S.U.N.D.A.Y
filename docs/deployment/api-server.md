...
SUNDAY includes an OpenAI-compatible API server built on FastAPI and uvicorn. It exposes chat completion, model listing, and health check endpoints, making it a drop-in replacement for the OpenAI API when working with local models.
...
The server reads defaults from `~/.sunday/config.toml` and auto-detects available engines and models. Override any option via CLI flags:
...
!!! warning "Server dependency check"
    If the `[server]` extra is not installed, `sunday serve` exits with a clear error message explaining how to install the required dependencies.
...
The `[server]` section of `~/.sunday/config.toml` controls default server behavior:
...
CLI flags override config file values. For example, `sunday serve --port 9000` overrides the `port` setting in the config file.
...
For production deployments, run SUNDAY behind a reverse proxy like Nginx or Caddy for TLS termination, rate limiting, and authentication.
...
server {
    listen 443 ssl;
    server_name sunday.example.com;

    ssl_certificate /etc/ssl/certs/sunday.pem;
    ssl_certificate_key /etc/ssl/private/sunday.key;
...
```bash
sunday serve --host 127.0.0.1 --port 8000
```
