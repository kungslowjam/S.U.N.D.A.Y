"""CLI for the SUNDAY evaluation framework."""
...
from sunday.evals.core.display import (
    print_banner,
    print_completion,
    print_full_results,
    print_run_header,
...
def _build_backend(
    ...
    """
    if backend_name == "sunday-agent":
        from sunday.evals.backends.sunday_agent import JarvisAgentBackend
    ...
    elif backend_name == "sunday-direct":
        from sunday.evals.backends.sunday_direct import JarvisDirectBackend
        ...
        from sunday.evals.backends.external import HermesBackend
        ...
        from sunday.evals.backends.external import OpenClawBackend
...
def _build_dataset(benchmark: str, subset: str | None = None):
    ...
    if benchmark == "supergpqa":
        from sunday.evals.datasets.supergpqa import SuperGPQADataset

        return SuperGPQADataset()
    elif benchmark == "gpqa":
        from sunday.evals.datasets.gpqa import GPQADataset

        return GPQADataset()
    elif benchmark == "mmlu-pro":
        from sunday.evals.datasets.mmlu_pro import MMLUProDataset

        return MMLUProDataset()
    elif benchmark == "math500":
        from sunday.evals.datasets.math500 import MATH500Dataset

        return MATH500Dataset()
    elif benchmark == "natural-reasoning":
        from sunday.evals.datasets.natural_reasoning import NaturalReasoningDataset

        return NaturalReasoningDataset()
    elif benchmark == "hle":
        from sunday.evals.datasets.hle import HLEDataset

        return HLEDataset()
    elif benchmark == "simpleqa":
        from sunday.evals.datasets.simpleqa import SimpleQADataset

        return SimpleQADataset()
    elif benchmark == "wildchat":
        from sunday.evals.datasets.wildchat import WildChatDataset

        return WildChatDataset()
    elif benchmark == "ipw":
        from sunday.evals.datasets.ipw_mixed import IPWDataset

        return IPWDataset()
    elif benchmark == "gaia":
        from sunday.evals.datasets.gaia import GAIADataset

        return GAIADataset()
    elif benchmark == "frames":
        from sunday.evals.datasets.frames import FRAMESDataset

        return FRAMESDataset()
    elif benchmark == "swebench":
        from sunday.evals.datasets.swebench import SWEBenchDataset

        return SWEBenchDataset()
    elif benchmark == "swefficiency":
        from sunday.evals.datasets.swefficiency import SWEfficiencyDataset

        return SWEfficiencyDataset()
    elif benchmark == "terminalbench":
        from sunday.evals.datasets.terminalbench import TerminalBenchDataset

        return TerminalBenchDataset()
    elif benchmark == "terminalbench-native":
        from sunday.evals.datasets.terminalbench_native import (
            TerminalBenchNativeDataset,
        )

        return TerminalBenchNativeDataset()
    elif benchmark == "email_triage":
        from sunday.evals.datasets.email_triage import EmailTriageDataset

        return EmailTriageDataset()
    elif benchmark == "morning_brief":
        from sunday.evals.datasets.morning_brief import MorningBriefDataset

        return MorningBriefDataset()
    elif benchmark == "research_mining":
        from sunday.evals.datasets.research_mining import ResearchMiningDataset

        return ResearchMiningDataset()
    elif benchmark == "knowledge_base":
        from sunday.evals.datasets.knowledge_base import KnowledgeBaseDataset

        return KnowledgeBaseDataset()
    elif benchmark == "coding_task":
        from sunday.evals.datasets.coding_task import CodingTaskDataset

        return CodingTaskDataset()
    elif benchmark == "loghub":
        from sunday.evals.datasets.loghub import LogHubDataset

        return LogHubDataset()
    elif benchmark == "ama-bench":
        from sunday.evals.datasets.ama_bench import AMABenchDataset

        return AMABenchDataset()
    elif benchmark == "lifelong-agent":
        from sunday.evals.datasets.lifelong_agent import LifelongAgentDataset

        return LifelongAgentDataset(subset=subset or "db_bench")
    elif benchmark == "deepplanning":
        from sunday.evals.datasets.deepplanning import DeepPlanningDataset

        return DeepPlanningDataset()
    elif benchmark == "paperarena":
        from sunday.evals.datasets.paperarena import PaperArenaDataset

        return PaperArenaDataset()
    elif benchmark == "webchorearena":
        from sunday.evals.datasets.webchorearena import WebChoreArenaDataset

        return WebChoreArenaDataset()
    elif benchmark == "workarena":
        from sunday.evals.datasets.workarena import WorkArenaDataset

        return WorkArenaDataset()
    elif benchmark == "coding_assistant":
        from sunday.evals.datasets.coding_assistant import CodingAssistantDataset

        return CodingAssistantDataset()
    elif benchmark == "security_scanner":
        from sunday.evals.datasets.security_scanner import SecurityScannerDataset

        return SecurityScannerDataset()
    elif benchmark == "daily_digest":
        from sunday.evals.datasets.daily_digest import DailyDigestDataset

        return DailyDigestDataset()
    elif benchmark == "doc_qa":
        from sunday.evals.datasets.doc_qa import DocQADataset

        return DocQADataset()
    elif benchmark == "browser_assistant":
        from sunday.evals.datasets.browser_assistant import BrowserAssistantDataset

        return BrowserAssistantDataset()
    elif benchmark == "pinchbench":
        from sunday.evals.datasets.pinchbench import PinchBenchDataset

        return PinchBenchDataset(path=subset)
    elif benchmark == "taubench":
        from sunday.evals.datasets.taubench import TauBenchDataset

        domains = subset.split(",") if subset else None
        return TauBenchDataset(domains=domains)
    elif benchmark == "livecodebench":
        from sunday.evals.datasets.livecodebench import LiveCodeBenchDataset

        return LiveCodeBenchDataset()
    elif benchmark in ("liveresearch", "deepresearch"):
        from sunday.evals.datasets.liveresearch import LiveResearchBenchDataset

        return LiveResearchBenchDataset(path=subset)
    elif benchmark == "liveresearchbench":
        from sunday.evals.datasets.liveresearchbench import (
            LiveResearchBenchDataset as LRBDataset,
        )

        return LRBDataset()
    elif benchmark == "toolcall15":
        from sunday.evals.datasets.toolcall15 import ToolCall15Dataset
...
def _build_scorer(benchmark: str, judge_backend, judge_model: str):
    ...
    if benchmark == "supergpqa":
        from sunday.evals.scorers.supergpqa_mcq import SuperGPQAScorer

        return SuperGPQAScorer(judge_backend, judge_model)
    elif benchmark == "gpqa":
        from sunday.evals.scorers.gpqa_mcq import GPQAScorer

        return GPQAScorer(judge_backend, judge_model)
    elif benchmark == "mmlu-pro":
        from sunday.evals.scorers.mmlu_pro_mcq import MMLUProScorer

        return MMLUProScorer(judge_backend, judge_model)
    elif benchmark == "math500" or benchmark == "natural-reasoning":
        from sunday.evals.scorers.reasoning_judge import ReasoningJudgeScorer

        return ReasoningJudgeScorer(judge_backend, judge_model)
    elif benchmark == "hle":
        from sunday.evals.scorers.hle_judge import HLEScorer

        return HLEScorer(judge_backend, judge_model)
    elif benchmark == "simpleqa":
        from sunday.evals.scorers.simpleqa_judge import SimpleQAScorer

        return SimpleQAScorer(judge_backend, judge_model)
    elif benchmark == "wildchat":
        from sunday.evals.scorers.wildchat_judge import WildChatScorer

        return WildChatScorer(judge_backend, judge_model)
    elif benchmark == "ipw":
        from sunday.evals.scorers.ipw_mixed import IPWMixedScorer

        return IPWMixedScorer(judge_backend, judge_model)
    elif benchmark == "gaia":
        from sunday.evals.scorers.gaia_exact import GAIAScorer

        return GAIAScorer(judge_backend, judge_model)
    elif benchmark == "frames":
        from sunday.evals.scorers.frames_judge import FRAMESScorer

        return FRAMESScorer(judge_backend, judge_model)
    elif benchmark == "swebench":
        from sunday.evals.scorers.swebench_structural import SWEBenchScorer

        return SWEBenchScorer(judge_backend, judge_model)
    elif benchmark == "swefficiency":
        from sunday.evals.scorers.swefficiency_structural import SWEfficiencyScorer

        return SWEfficiencyScorer(judge_backend, judge_model)
    elif benchmark == "terminalbench":
        from sunday.evals.scorers.terminalbench_judge import TerminalBenchScorer

        return TerminalBenchScorer(judge_backend, judge_model)
    elif benchmark == "terminalbench-native":
        from sunday.evals.scorers.terminalbench_native_structural import (
            TerminalBenchNativeScorer,
        )

        return TerminalBenchNativeScorer(judge_backend, judge_model)
    elif benchmark == "email_triage":
        from sunday.evals.scorers.email_triage import EmailTriageScorer

        return EmailTriageScorer(judge_backend, judge_model)
    elif benchmark == "morning_brief":
        from sunday.evals.scorers.morning_brief import MorningBriefScorer

        return MorningBriefScorer(judge_backend, judge_model)
    elif benchmark == "research_mining":
        from sunday.evals.scorers.research_mining import ResearchMiningScorer

        return ResearchMiningScorer(judge_backend, judge_model)
    elif benchmark == "knowledge_base":
        from sunday.evals.scorers.knowledge_base import KnowledgeBaseScorer

        return KnowledgeBaseScorer(judge_backend, judge_model)
    elif benchmark == "coding_task":
        from sunday.evals.scorers.coding_task import CodingTaskScorer

        return CodingTaskScorer(judge_backend, judge_model)
    elif benchmark == "loghub":
        from sunday.evals.scorers.loghub_scorer import LogHubScorer

        return LogHubScorer(judge_backend, judge_model)
    elif benchmark == "ama-bench":
        from sunday.evals.scorers.ama_bench_judge import AMABenchScorer

        return AMABenchScorer(judge_backend, judge_model)
    elif benchmark == "lifelong-agent":
        from sunday.evals.scorers.lifelong_agent_scorer import LifelongAgentScorer

        return LifelongAgentScorer(judge_backend, judge_model)
    elif benchmark == "deepplanning":
        from sunday.evals.scorers.deepplanning_scorer import DeepPlanningScorer

        return DeepPlanningScorer(judge_backend, judge_model)
    elif benchmark == "paperarena":
        from sunday.evals.scorers.paperarena_judge import PaperArenaScorer

        return PaperArenaScorer(judge_backend, judge_model)
    elif benchmark == "webchorearena":
        from sunday.evals.scorers.webchorearena_scorer import WebChoreArenaScorer

        return WebChoreArenaScorer(judge_backend, judge_model)
    elif benchmark == "workarena":
        from sunday.evals.scorers.workarena_scorer import WorkArenaScorer

        return WorkArenaScorer(judge_backend, judge_model)
    elif benchmark == "coding_assistant":
        from sunday.evals.scorers.coding_assistant import CodingAssistantScorer

        return CodingAssistantScorer(judge_backend, judge_model)
    elif benchmark == "security_scanner":
        from sunday.evals.scorers.security_scanner import SecurityScannerScorer

        return SecurityScannerScorer(judge_backend, judge_model)
    elif benchmark == "daily_digest":
        from sunday.evals.scorers.daily_digest import DailyDigestScorer

        return DailyDigestScorer(judge_backend, judge_model)
    elif benchmark == "doc_qa":
        from sunday.evals.scorers.doc_qa import DocQAScorer

        return DocQAScorer(judge_backend, judge_model)
    elif benchmark == "browser_assistant":
        from sunday.evals.scorers.browser_assistant import BrowserAssistantScorer

        return BrowserAssistantScorer(judge_backend, judge_model)
    elif benchmark == "pinchbench":
        from sunday.evals.scorers.pinchbench import PinchBenchScorer

        return PinchBenchScorer(judge_backend, judge_model)
    elif benchmark == "taubench":
        from sunday.evals.scorers.taubench import TauBenchScorer

        return TauBenchScorer(judge_backend, judge_model)
    elif benchmark == "livecodebench":
        from sunday.evals.scorers.livecodebench import LiveCodeBenchScorer

        return LiveCodeBenchScorer(judge_backend, judge_model)
    elif benchmark in ("liveresearch", "deepresearch"):
        from sunday.evals.scorers.liveresearch import LiveResearchBenchScorer

        return LiveResearchBenchScorer(judge_backend, judge_model)
    elif benchmark == "liveresearchbench":
        from sunday.evals.scorers.liveresearchbench import (
            LiveResearchBenchScorer as LRBScorer,
        )

        return LRBScorer(judge_backend, judge_model)
    elif benchmark == "toolcall15":
        from sunday.evals.scorers.toolcall15 import ToolCall15Scorer
...
def _build_trackers(config) -> list:
    ...
    if getattr(config, "wandb_project", ""):
        try:
            from sunday.evals.trackers.wandb_tracker import WandbTracker
    ...
    if getattr(config, "sheets_spreadsheet_id", ""):
        try:
            from sunday.evals.trackers.sheets_tracker import SheetsTracker
...
def _run_terminalbench_native(config, console: Console) -> object:
    ...
    from sunday.evals.backends.terminalbench_native import (
        TerminalBenchNativeBackend,
    )
    from sunday.evals.core.types import RunSummary
...
def _run_single(config, console: Optional[Console] = None) -> object:
    ...
    from sunday.evals.core.runner import EvalRunner
    ...
    eval_backend = _build_backend(
        ...
        base_url=(
            ...
            or os.environ.get("SUNDAY_BACKEND_BASE_URL")
        ),
        api_key=(
            getattr(config, "api_key", None)
            or _metadata.get("api_key")
            or os.environ.get("SUNDAY_BACKEND_API_KEY")
        ),
    )
...
def _run_agentic(
    ...
    *,
    ...
    from sunday.evals.core.agentic_runner import AgenticRunner
    from sunday.evals.core.event_recorder import EventRecorder
    from sunday.evals.core.export import (
    ...
    try:
        ...
        from sunday.telemetry.session import TelemetrySession
    ...
    try:
        from sunday.evals.core.export import export_hf_dataset
...
def _run_from_config(
    ...
    *,
    ...
    from sunday.evals.core.config import expand_suite, load_eval_config
...
@click.option(
    ...
    help="Agent name for sunday-agent backend",
)
...
def run(
    ...
    from sunday.evals.core.types import RunConfig
    ...
    config = RunConfig(
        ...
        base_url=base_url or os.environ.get("SUNDAY_BACKEND_BASE_URL"),
        api_key=api_key or os.environ.get("SUNDAY_BACKEND_API_KEY"),
        metadata={
            "base_url": base_url or os.environ.get("SUNDAY_BACKEND_BASE_URL"),
            "api_key": api_key or os.environ.get("SUNDAY_BACKEND_API_KEY"),
        },
...
def run_all(
    ...
    for i, bench_name in enumerate(BENCHMARKS, 1):
        ...
        eval_backend = _build_backend("sunday-direct", engine_key, "orchestrator", [])
...
def reparse_judge(jsonl_path, out_path, in_place, summary_out):
    ...
    from sunday.evals.scorers.liveresearch import rescore_from_metadata
