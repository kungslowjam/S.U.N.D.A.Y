...
from sunday.evals.core.backend import InferenceBackend
from sunday.evals.core.dataset import DatasetProvider
from sunday.evals.core.export import _hardware_info_dict
from sunday.evals.core.scorer import Scorer
from sunday.evals.core.tracker import ResultTracker
from sunday.evals.core.types import (
    EvalRecord,
    EvalResult,
    MetricStats,
    RunConfig,
...
try:
    from sunday.telemetry.efficiency import compute_efficiency
except ImportError:  # pragma: no cover
    compute_efficiency = None  # type: ignore[assignment]

try:
    from sunday.telemetry.efficiency import estimate_model_flops_per_token
except ImportError:  # pragma: no cover
...
class EvalRunner:
    ...
    def run(
        ...
        from sunday.evals.core.dataset import DatasetProvider as _DP
    ...
    def _process_one(self, record: EvalRecord) -> EvalResult:
        ...
        def _backend_error_result(full: dict, message: str) -> EvalResult:
            ...
            return EvalResult(
                ...
                framework=full.get(
                    "framework",
                    getattr(self._backend, "framework_name", "sunday"),
                ),
        ...
        try:
            ...
            return EvalResult(
                ...
                framework=full.get("framework", "sunday"),
            ...
            return EvalResult(
                ...
                model_answer="",
                error=str(exc),
                framework=getattr(self._backend, "framework_name", "sunday"),
