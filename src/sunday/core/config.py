...
User configuration lives at ``~/.sunday/config.toml``.  ``load_config()``
...
DEFAULT_CONFIG_DIR = Path.home() / ".sunday"
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from sunday.intelligence.model_catalog import BUILTIN_MODELS
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.sunday/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through SUNDAY. You are not a cloud service. Respond "
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.sunday/whatsapp_auth
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "sunday-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.sunday/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.sunday/operators"
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.sunday/SOUL.md"
    memory_path: str = "~/.sunday/MEMORY.md"
    user_path: str = "~/.sunday/USER.md"
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.sunday/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/sunday/skill-index.git"
    index_dir: str = "~/.sunday/skill-index/"
    max_depth: int = 5
...
@dataclass
class JarvisConfig:
    """Top-level configuration for SUNDAY."""
...
# Sections that users may set via ``sunday config set``.
...
def validate_config_key(dotted_key: str) -> type:
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import sunday.core.config as _cfg_mod
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``sunday init``)
# ---------------------------------------------------------------------------
...
# Full reference config: sunday init --full
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.sunday/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.sunday/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.sunday/whatsapp_auth
