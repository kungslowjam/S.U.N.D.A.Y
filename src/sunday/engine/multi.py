...
class MultiEngine(InferenceEngine):
    ...
    def _engine_for(self, model: str) -> InferenceEngine:
        ...
        # Clean the model name (remove UI prefixes if any)
        clean_model = model
        if " - " in model:
            # Handle cases like "ollama - model_name"
            clean_model = model.split(" - ", 1)[-1].strip()

        engine = self._model_map.get(clean_model) or self._model_map.get(model)
        ...
        # If model is a .gguf file, route to llamacpp engine
        if clean_model.endswith(".gguf") or model.endswith(".gguf"):
            for key, eng in self._engines:
                if key == "llamacpp":
                    logger.info("Forcing .gguf model %r to llamacpp engine (Port 8081)", model)
                    return eng
            logger.warning("Model %r looks like GGUF but llamacpp engine is not registered", model)

        # If model looks like a cloud model, route to the cloud engine
        # rather than falling back to the local engine (which would 404).
        if any(model.startswith(p) for p in self._CLOUD_PREFIXES) or any(clean_model.startswith(p) for p in self._CLOUD_PREFIXES):
            for key, eng in self._engines:
                if key == "cloud":
                    logger.info("Routing cloud model %r to cloud engine", model)
                    return eng

        # Non-cloud models: do NOT silently fall back to cloud.
        logger.error("No suitable engine found for model %r (Cleaned: %r)", model, clean_model)
        raise ValueError(
            f"Model {model!r} not found in any engine "
            f"(known: {', '.join(sorted(self._model_map.keys())) or '<none>'}). "
            f"Check that the expected backend (e.g. llama-server on 8081) is reachable."
        )
