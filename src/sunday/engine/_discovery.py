...
def _make_engine(key: str, config: JarvisConfig) -> InferenceEngine:
    ...
    if host_attr is not None:
        ...
        if host:
            # SAFETY: Explicitly block Port 8080 to avoid conflicts with AIRDE
            if ":8080" in str(host):
                logger.debug("Blocking discovery of engine %r on port 8080", key)
                raise ValueError("Port 8080 is blacklisted in SUNDAY to avoid conflicts with AIRDE.")
