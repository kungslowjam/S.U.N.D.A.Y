"""MCP Server — wraps SUNDAY tools as MCP-discoverable tools."""
...
from sunday.mcp.protocol import (
    INTERNAL_ERROR,
    INVALID_PARAMS,
    METHOD_NOT_FOUND,
    MCPRequest,
...
class MCPServer:
    """MCP server that exposes SUNDAY tools via JSON-RPC.
    ...
    def _auto_discover_tools() -> List[BaseTool]:
        ...
        try:
            ...
            pass
        try:
            from sunday.tools.code_interpreter import CodeInterpreterTool

            _tool_classes.append(CodeInterpreterTool)
        except ImportError:
            pass
        try:
            from sunday.tools.repl import ReplTool
        ...
        # Storage MCP tools
        try:
            from sunday.tools.storage_tools import (
                MemoryIndexTool,
                MemoryRetrieveTool,
                MemorySearchTool,
                MemoryStoreTool,
        ...
        # Channel MCP tools
        try:
            from sunday.tools.channel_tools import (
                ChannelListTool,
                ChannelSendTool,
                ChannelStatusTool,
            )
