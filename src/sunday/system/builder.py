...
from sunday.core.events import EventBus, get_event_bus
...
class SystemBuilder:
    ...
    def _resolve_memory(self, config):
        try:
            import sunday.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import sunday.channels  # noqa: F401 -- trigger registration
            from sunday.core.registry import ChannelRegistry
            from sunday.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from sunday.sandbox.runner import ContainerRunner
    ...
    def _setup_learning_orchestrator(config: JarvisConfig):
        ...
        try:
            from sunday.core.config import DEFAULT_CONFIG_DIR
            from sunday.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from sunday.learning.training.lora import LoRATrainingConfig
            from sunday.traces.store import TraceStore
