"""Load system prompt and few-shot overrides from $OPENSUNDAY_HOME.
...
- System prompts: ``$OPENSUNDAY_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$OPENSUNDAY_HOME/agents/{name}/few_shot.json``
...
def _sunday_home() -> Path:
    """Resolve $OPENSUNDAY_HOME, defaulting to ~/.sunday."""
    return Path(os.environ.get("OPENSUNDAY_HOME", "~/.sunday")).expanduser()


def load_system_prompt_override(agent_name: str) -> str | None:
    """Return the override prompt for *agent_name*, or ``None``.

    Looks for ``$OPENSUNDAY_HOME/agents/<agent_name>/system_prompt.md``.
    ``OPENSUNDAY_HOME`` defaults to ``~/.sunday`` when unset.
    """
    home = _sunday_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$OPENSUNDAY_HOME/agents/<agent_name>/few_shot.json``.
