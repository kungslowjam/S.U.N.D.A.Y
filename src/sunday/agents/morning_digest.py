...
from sunday.agents.digest_store import DigestArtifact, DigestStore
from sunday.core.registry import AgentRegistry
from sunday.core.types import Message, Role, ToolCall


def _load_persona(persona_name: str) -> str:
    """Load a persona prompt file by name."""
    search_paths = [
        Path("configs/sunday/prompts/personas") / f"{persona_name}.md",
        Path.home() / ".sunday" / "prompts" / "personas" / f"{persona_name}.md",
    ]
...
class MorningDigestAgent(ToolUsingAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._persona = kwargs.pop("persona", "sunday")
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        evaluator_feedback = ""
        try:
            from sunday.agents.digest_evaluator import DigestEvaluator
