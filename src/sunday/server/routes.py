...
from sunday.server.models import (
    ChatCompletionChunk,
...
def _handle_agent(
    ...
    if audio_path:
        ...
        from sunday.server.models import AudioMeta
...
async def _handle_agent_stream(agent, bus, model, req):
    ...
    from sunday.server.stream_bridge import create_agent_stream
...
async def _handle_stream(
    ...
    from sunday.server.cloud_router import (
        is_cloud_model,
        stream_cloud,
        stream_local,
    )
...
async def list_models(request: Request) -> ModelListResponse:
    ...
    """
    from sunday.server.cloud_router import is_cloud_model, list_local_models

    # Merge models from the active engine and the local models directory
    engine = request.app.state.engine
    try:
    ...
    # Filter out cloud models from engine list
    local_engine_models = [m for m in engine_models if not is_cloud_model(m)]

    # Get models from the configured local directory (GGUF files) and Ollama
    discovered_models = await list_local_models()

    # Combine both lists, ensuring uniqueness
    combined_ids = sorted(list(set(local_engine_models + discovered_models)))

    return ModelListResponse(
        data=[ModelObject(id=mid) for mid in combined_ids],
    )
...
async def reload_cloud_engine(request: Request):
    ...
    # Re-read ~/.sunday/cloud-keys.env and update the running process env.
    keys_path = Path.home() / ".sunday" / "cloud-keys.env"
    if keys_path.exists():
    ...
    try:
        from sunday.engine.cloud import CloudEngine
        from sunday.engine.multi import MultiEngine
...
async def security_scan():
    ...
    from sunday.cli.scan_cmd import PrivacyScanner
