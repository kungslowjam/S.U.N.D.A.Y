...
logger = logging.getLogger("sunday.server.agent_manager")
...
def _ensure_registries_populated() -> None:
    ...
    from sunday.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import sunday.channels  # noqa: F401
    except Exception:
        pass

    try:
        import sunday.tools  # noqa: F401
    except Exception:
        pass

    # Also try to import browser tools (not included in sunday.tools.__init__)
    for _browser_mod in ("sunday.tools.browser", "sunday.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("sunday.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("sunday.tools.")
    ...
    if not any(ToolRegistry.contains(n) for n in _BROWSER_SUB_TOOLS):
        for _browser_mod in (
            "sunday.tools.browser",
            "sunday.tools.browser_axtree",
        ):
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from sunday.core.credentials import TOOL_CREDENTIALS
    from sunday.core.registry import ChannelRegistry, ToolRegistry
...
def _build_deep_research_tools(
    ...
    from sunday.tools.knowledge_sql import KnowledgeSQLTool
    from sunday.tools.scan_chunks import ScanChunksTool
    from sunday.tools.think import ThinkTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from sunday.mcp.transport import StdioTransport, StreamableHTTPTransport
    from sunday.tools.mcp_adapter import MCPToolProvider
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from sunday.core.types import ToolCall as MsgToolCall
                ...
                for tc in sorted_tcs:
                    ...
                    try:
                        ...
                        if mcp_adapter is not None:
                            ...
                            from sunday.tools._stubs import (
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from sunday.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            ...
                            if tools:
                                from sunday.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from sunday.channels.sendblue import (
                        SendBlueChannel,
                    )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from sunday.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from sunday.channels.slack_daemon import (
    ...
    def trigger_learning(agent_id: str):
        if not manager.get_agent(agent_id):
            raise HTTPException(status_code=404, detail="Agent not found")
        from sunday.core.events import EventType, get_event_bus
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            ...
            config = load_config()
            store = TraceStore(config.traces.db_path or "~/.sunday/traces.db")
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from sunday.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from sunday.core.credentials import get_credential_status
