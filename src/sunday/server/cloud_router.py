...
Reads API keys from ~/.sunday/cloud-keys.env at request time so
...
_CLOUD_ENV_FILE = Path.home() / ".sunday" / "cloud-keys.env"
...
async def list_local_models() -> list[str]:
    """Return Ollama model names and local GGUF files from models_dir."""
    models = []
    ...
    # 2. Get local GGUF models dynamically from project structure
    try:
        # Try to find the project root relative to this file (src/sunday/server/cloud_router.py)
        # Root is 3 levels up
        project_root = Path(__file__).parents[3]
        possible_paths = [
            project_root / "llama-cpp" / "models",
            Path.cwd() / "llama-cpp" / "models",
            Path.home() / ".sunday" / "models"
        ]

        # Also check config as a fallback
        from sunday.core.config import load_config
        config = load_config()
        config_models_dir = getattr(config.engine.llamacpp, "models_dir", None)
        if config_models_dir:
            possible_paths.append(Path(config_models_dir.replace("\\", "/")).expanduser())

        found_gguf = []
        for p in possible_paths:
            if p.exists() and p.is_dir():
                for f in p.glob("*.gguf"):
                    found_gguf.append(f.name)

        if found_gguf:
            models.extend(found_gguf)
    except Exception:
        pass

    # Unique and sorted
    final_list = sorted(list(set(models)))
    return final_list
