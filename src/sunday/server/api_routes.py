...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import sunday.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from sunday.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from sunday.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from sunday.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from sunday.tools.agent_tools import AgentSendTool
...
def _get_memory_backend(request: Request):
    ...
    if backend is None:
        try:
            from sunday.tools.storage.sqlite import SQLiteMemory
...
async def list_skills(request: Request):
    ...
    try:
        ...
        skill_paths = [Path("./skills"), Path("~/.sunday/skills/").expanduser()]
        mgr = SkillManager(bus=EventBus())
        mgr.discover(paths=[p for p in skill_paths if p.exists()])
...
async def install_skill(req: SkillInstallRequest, request: Request):
    """Install a skill from a source (hermes, openclaw, github)."""
    try:
        from pathlib import Path
        ...
        if req.source == "hermes":
            ...
            if not req.url:
                raise HTTPException(
                    status_code=400,
                    detail="github source requires --url",
                )
            cache_root = req.url.rstrip("/").rsplit("/", 1)[-1]
            cache = Path(f"~/.sunday/skill-cache/github/{cache_root}").expanduser()
            resolver = GitHubResolver(cache_root=cache, repo_url=req.url)
        else:
            raise HTTPException(status_code=400, detail=f"Unknown source: {req.source}")

        resolver.sync()
        matches = [s for s in resolver.list_skills() if s.name == req.name]
        if not matches:
            raise HTTPException(
                status_code=404,
        ...
        logger.warning("Failed to install skill: %s", exc)
        raise HTTPException(status_code=500, detail=str(exc))
...
async def remove_skill(skill_name: str, request: Request):
    ...
    try:
        ...
        removed = mgr.remove(skill_name, roots=skill_paths)
        return {"success": True, "removed": [str(p) for p in removed]}
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail=f"Skill '{skill_name}' not found")
    except Exception as exc:
        logger.warning("Failed to remove skill: %s", exc)
        raise HTTPException(status_code=500, detail=str(exc))
    ...
    try:
        ...
        for src_cfg in sources_to_search:
            if src_cfg.source == "hermes":
                ...
                if not src_cfg.url:
                    continue
                cache_root = src_cfg.url.rstrip("/").rsplit("/", 1)[-1]
                cache = Path(f"~/.sunday/skill-cache/github/{cache_root}").expanduser()
                resolver = GitHubResolver(cache_root=cache, repo_url=src_cfg.url)
            else:
                continue
        ...
        logger.warning("Failed to list available skills: %s", exc)
        return {"skills": []}
...
async def sync_skills(
    ...
    """Bulk sync skills from configured sources."""
    try:
        from pathlib import Path
        ...
        for src in source_configs:
            if src.source == "hermes":
                ...
                if not src.url:
                    continue
                cache_root = src.url.rstrip("/").rsplit("/", 1)[-1]
                cache = Path(f"~/.sunday/skill-cache/github/{cache_root}").expanduser()
                resolver = GitHubResolver(cache_root=cache, repo_url=src.url)
            else:
                continue
            ...
            for resolved in resolver.list_skills():
                r = importer.import_skill(
                    resolved, with_scripts=with_scripts, force=force,
                )
        ...
        return {"installed": total_installed, "skipped": total_skipped}
    except HTTPException:
        raise
    except Exception as exc:
        logger.warning("Failed to sync skills: %s", exc)
        raise HTTPException(status_code=500, detail=str(exc))
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from sunday.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP sunday_requests_total Total requests processed",
            "# TYPE sunday_requests_total counter",
            f"sunday_requests_total {stats.get('total_requests', 0)}",
            "# HELP sunday_tokens_total Total tokens generated",
            "# TYPE sunday_tokens_total counter",
            f"sunday_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP sunday_latency_avg_ms Average latency in milliseconds",
            "# TYPE sunday_latency_avg_ms gauge",
            f"sunday_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
def include_all_routes(app) -> None:
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from sunday.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    try:
        ...
        from sunday.server.ws_bridge import create_ws_router
