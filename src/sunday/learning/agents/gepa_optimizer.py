...
Uses GEPA's adapter pattern to bridge SUNDAY traces into GEPA's
...
from sunday.core.config import GEPAOptimizerConfig
from sunday.core.registry import LearningRegistry
from sunday.learning._stubs import AgentLearningPolicy
...
class SUNDAYGEPAAdapter:
    """Implements GEPA's adapter protocol for SUNDAY agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'sunday[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = SUNDAYGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "SUNDAYGEPAAdapter"]
