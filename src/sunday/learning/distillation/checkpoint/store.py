...
A thin wrapper over a local git repository at ``<sunday_home>/.git``.
...
from sunday.learning.distillation.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.sunday/`` (or ``$OPENSUNDAY_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the SUNDAY source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"SUNDAY source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "distillation@sunday.local")
            self._git("config", "user.name", "SUNDAY Distillation")
        ...
        # Allow empty so init succeeds even on a brand-new sunday home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
