...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.sunday.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "sunday_meta" the value is
# stored under manifest.metadata["sunday"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("sunday_meta", "platforms"),
    "prerequisites": ("sunday_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.sunday.original_frontmatter,
        and merges metadata.sunday.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize sunday namespace
        oj_meta = dict(raw_metadata.get("sunday") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.sunday.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.sunday.* into canonical fields (these override
        # top-level mappings since they are explicit SUNDAY-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.sunday.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the sunday metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["sunday"] = oj_meta
