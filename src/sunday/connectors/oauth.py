...
class OAuthProvider:
    ...
    # Filenames in ~/.sunday/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.sunday/connectors/`` and falls
    back to environment variables ``OPENSUNDAY_{NAME}_CLIENT_ID`` and
    ``OPENSUNDAY_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"OPENSUNDAY_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to SUNDAY.</p>"
                    b"</body></html>"
                )
