"""``sunday start|stop|restart|status`` — daemon management commands."""
...
from sunday.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the SUNDAY server daemon."""
...
def start(
    ...
    """Start the SUNDAY server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'sunday stop' to stop it first, or 'sunday restart'.")
        sys.exit(1)
    ...
    # Build command to run sunday serve
    cmd = [sys.executable, "-m", "sunday.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]SUNDAY server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running SUNDAY server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the SUNDAY server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the SUNDAY server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
