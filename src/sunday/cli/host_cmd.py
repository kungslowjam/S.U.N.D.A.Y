"""``sunday host`` — download and serve a model locally with auto backend setup."""
...
def _build_serve_command(backend: str, model: str, port: int, gpu_layers: int = 0) -> list[str]:
    ...
    if backend == "llamacpp":
        cmd = ["llama-server", "-m", model, "--port", str(port)]
        if gpu_layers > 0:
...
@click.option(
    ...
    type=int,
    default=0,
    help="Number of layers to offload to GPU (llama.cpp only).",
)
def host(
    model: str,
    ...
    \b
      sunday host mlx-community/Qwen2.5-7B-4bit --backend mlx
      sunday host Qwen/Qwen3-8B --backend vllm
      sunday host qwen3:8b --backend ollama
      sunday host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    cmd = _build_serve_command(backend, model, serve_port, gpu_layers)
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]SUNDAY will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
