"""``sunday doctor`` — run diagnostic checks on the SUNDAY installation."""
...
from sunday.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `sunday init` to generate a config file.",
    )
...
def _ensure_engines_imported() -> None:
    ...
    try:
        import sunday.engine  # noqa: F401
    except Exception:
        pass
...
def _check_engines() -> List[CheckResult]:
    ...
    from sunday.engine import _discovery
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "sunday[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "sunday[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "sunday[energy-amd]", "AMD energy monitoring"),
        ("colbert", "sunday[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "sunday[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your SUNDAY installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]SUNDAY Doctor[/bold]")
    console.print()
    ...
    from sunday.cli._bg_state import get_status
    ...
    if bg.rust_extension == "ready":
        ...
        console.print(
            "    retry: ~/.sunday/.scripts/install-rust.sh && "
            "~/.sunday/.scripts/build-extension.sh"
        )
        bg_failed = True
    else:
        console.print(
    ...
    for model_id, state in bg.models.items():
        if state == "ready":
            ...
            console.print(f"    retry: ~/.sunday/.scripts/pull-model.sh {model_id}")
            bg_failed = True
        else:
