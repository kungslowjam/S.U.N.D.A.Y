"""``sunday eval`` — evaluation framework CLI commands."""
...
@click.option(
    ...
    type=click.Choice(
        ["sunday-direct", "sunday-agent", "hermes", "openclaw", "terminalbench-native"]
    ),
    help=(
        "Inference backend. For hermes/openclaw, also pass --base-url and "
        "--api-key (or set SUNDAY_BACKEND_BASE_URL/SUNDAY_BACKEND_API_KEY)."
    ),
)
@click.option(
    "--base-url",
...
@click.option(
    ...
    "api_key",
    default=None,
    help=("API key for the hermes/openclaw endpoint (env: SUNDAY_BACKEND_API_KEY)."),
)
@click.option(
    "--agent",
    "agent_name",
...
def eval_run(
    ...
    if config_path is not None:
        ...
        try:
            from sunday.evals.cli import _run_single
        except ImportError:
    ...
    config = RunConfig(
        ...
        # so users can also set SUNDAY_BACKEND_BASE_URL/SUNDAY_BACKEND_API_KEY.
