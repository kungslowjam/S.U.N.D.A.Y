"""``sunday init`` — detect hardware, generate config, write to disk."""
...
from sunday.cli._bootstrap import detect_cloud_keys
from sunday.cli.model import find_model_spec, hf_download, ollama_pull
...
# Engines supported by ``sunday init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     sunday ask "Hello"\n'
            "\n"
            "  Run `sunday doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]sunday scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-sunday first-run guard.",
)
def init(
    force: bool,
    ...
    from_bare_sunday: bool = False,
) -> None:
    """Detect hardware and generate ~/.sunday/config.toml."""
    console = Console()
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "sunday" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]sunday doctor[/bold] to verify."
        )
        return
    ...
    if engine is None and config is None:
        ...
        # Bare-sunday cold path: use the recommended engine non-interactively.
        if from_bare_sunday:
            engine = recommended
        else:
            console.print()
...
"""
        ...
        console.print(
            ...
            "Run [bold]sunday connect gdrive[/bold] to connect "
            "Google services, then [bold]sunday digest --fresh[/bold]."
        )
    ...
    if not model:
        ...
        if not no_download and not from_bare_sunday and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]sunday model pull {model}[/bold]"
                )
