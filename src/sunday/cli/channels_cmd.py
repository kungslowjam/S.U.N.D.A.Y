"""``sunday channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from sunday.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "sunday channels imessage-start <contact>",
        )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "sunday.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from sunday.connectors.retriever import (
            TwoStageRetriever,
        )
        from sunday.connectors.store import KnowledgeStore
        from sunday.engine.ollama import OllamaEngine
        from sunday.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from sunday.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from sunday.channels.imessage_daemon import stop_daemon
