...
from sunday.core.config import HardwareInfo, detect_hardware, load_config
...
from sunday.mining._models import (
    get_pearl_model_spec,
    iter_pearl_model_specs,
    pearl_variant_for_base_model,
)
from sunday.mining._stubs import Sidecar
from sunday.mining.vllm_pearl import ensure_registered as ensure_vllm_registered
...
def _ensure_providers_registered() -> None:
    ...
    from sunday.mining.cpu_pearl import ensure_registered as ensure_cpu_registered

    ensure_cpu_registered()
    try:
        from sunday.mining.apple_mps_pearl import (
...
def doctor() -> None:
    ...
    if mining_cfg is None:
        _row("RPC", False, "no [mining] config - run `sunday mine init`")
    else:
...
def init(
    ...
    if selected_provider == "vllm-pearl":
        ...
        if not cap.supported:
            raise click.ClickException(
                ...
                "See `sunday mine models` and `sunday mine doctor` for details."
            )
    ...
    if pearld_password_env not in os.environ:
        click.echo(
            ...
            "Set it before `sunday mine start`.",
            err=True,
        )

    from sunday.core.config import DEFAULT_CONFIG_PATH

    config_path = Path(os.environ.get("OPENSUNDAY_CONFIG", DEFAULT_CONFIG_PATH))
...
"""
    ...
    click.echo("Done. Run `sunday mine start` to begin mining.")
...
def start() -> None:
    ...
    if cfg is None:
        raise click.ClickException(
            "no [mining] section in config - run `sunday mine init`"
        )
    provider = MinerRegistry.get(cfg.provider)()
    asyncio.run(provider.start(cfg))
    click.echo(f"Started {cfg.provider}. Run `sunday mine status` for live stats.")
...
def validate_model(
    ...
    if sidecar is None:
        record(
            "Sidecar",
            False,
            "absent - run `sunday mine start` first",
        )
    else:
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        launcher._container = client.containers.get("sunday-pearl-miner")
    except Exception as exc:  # noqa: BLE001
