"""``sunday deep-research-setup`` — auto-detect local sources, ingest, and chat.
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from sunday.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from sunday.connectors.imessage import IMessageConnector
        ...
        from sunday.connectors.gmail_imap import GmailIMAPConnector

        return GmailIMAPConnector()
    elif connector_id == "outlook":
        from sunday.connectors.outlook import OutlookConnector

        return OutlookConnector()
    elif connector_id == "slack":
        from sunday.connectors.slack_connector import SlackConnector

        return SlackConnector()
    elif connector_id == "notion":
        from sunday.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from sunday.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.sunday/sync_state.db`` when empty.
