...
Used by both ``install.sh`` (via ``sunday _bootstrap --write-config``)
and ``sunday init`` (so there is a single source of truth for the
...
import sunday
from sunday.core import config as _cfg
...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``sunday _bootstrap --write-config``)
    and ``sunday init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        f"# SUNDAY configuration\n"
        f"{hardware_line}{gpu_comment}\n"
        f"# Full reference config: sunday init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command — invoked by install.sh, hidden from `sunday --help`
# ---------------------------------------------------------------------------
