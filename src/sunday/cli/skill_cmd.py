...
def _get_discovered_dir() -> Path:
    ...
    return Path("~/.sunday/skills/discovered/").expanduser()


def _get_overlay_dir() -> Path:
    """Return the directory where optimization overlays are stored."""
    return Path("~/.sunday/learning/skills/").expanduser()
...
def _get_skill_paths() -> List[Path]:
    ...
    user_dir = Path("~/.sunday/skills/").expanduser()
...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "github":
        ...
        cache = _Path(
            "~/.sunday/skill-cache/github/" + url.rstrip("/").rsplit("/", 1)[-1]
        ).expanduser()
...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``sunday skill install hermes:apple-notes``
    """
    console = Console()
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.sunday/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: sunday skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.sunday/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="sunday-discover-dryrun-"))
        try:
