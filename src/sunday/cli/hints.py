...
def hint_no_config() -> str:
    ...
    return (
        ...
        "  Run [bold]sunday init[/bold] to detect hardware and generate "
        "[cyan]~/.sunday/config.toml[/cyan].\n"
        "  Or run [bold]sunday quickstart[/bold] for a guided setup."
    )
...
def hint_no_engine(engine_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "  Run [bold]sunday doctor[/bold] to check all engines.\n"
        "  Run [bold]sunday quickstart[/bold] for guided setup.\n"
        "\n"
        "  [dim]To use a remote engine:[/dim]\n"
        f"    [cyan]sunday config set engine.{name}.host http://<remote-ip>:<port>[/cyan]\n"
...
def hint_no_model(model_name: Optional[str] = None) -> str:
    ...
    if model_name:
        return (
            ...
            "  Run [bold]sunday model list[/bold] to see available models."
        )
    return (
