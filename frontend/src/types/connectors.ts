...
export const SOURCE_CATALOG: ConnectorMeta[] = [
  ...
  // ── Communication ──────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        label: 'Go to App Passwords. Select app: "Mail", device: "Other" and type "SUNDAY". Click Generate. This does NOT open a login popup \u2014 you\'ll get a 16-character password to copy (you won\'t see it again).',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to api.slack.com/apps and click "Create New App" → choose "From scratch". Name it "SUNDAY" and pick your workspace',
      ...
      {
        label: 'Paste the bot token below. After connecting, invite the bot to channels you want indexed by typing /invite @SUNDAY in each channel',
      },
      {
        label: '(Optional) Set the app icon: in the left sidebar click "Basic Information" → scroll to "Display Information" → upload the SUNDAY logo',
        url: 'https://github.com/open-sunday/SUNDAY/blob/main/assets/sunday-slack-icon.jpg',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to notion.so/profile/integrations → click "+ New integration". Name it "SUNDAY", select your workspace, and click Submit',
      ...
      {
        label: 'To share ALL your pages at once: open any top-level page → click "..." (top right) → "Connections" → "Add connections" → search "SUNDAY" → click it. This shares the page and all its sub-pages. Repeat for each top-level page, or share your entire workspace by doing this on every root page',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Click the "+" button at the bottom of the list. Navigate to Applications → Utilities → select "Terminal.app" (or iTerm2/Warp if you use those). If you\'re using the desktop app, also add "SUNDAY.app" from Applications',
      },
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart SUNDAY). iMessage data will be detected automatically — no credentials needed',
      },
    ],
  },
  // ── Documents ──────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        label: 'Paste the full path below. SUNDAY will index all .md files in the vault',
      },
    ],
    inputFields: [
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to Google Cloud Console → create a new project (or select an existing one). Give it any name (e.g. "SUNDAY")',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart SUNDAY). Apple Notes will be detected automatically — no credentials needed',
      },
    ],
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart SUNDAY). Apple Contacts will be detected automatically — no credentials needed',
      },
    ],
  },
  {
    ...
    steps: [
      {
        label: 'Go to the Azure Portal → App Registrations → click "+ New registration". Name it "SUNDAY", select "Accounts in this organizational directory only", and click Register',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to the Dropbox App Console and click "Create app". Choose "Scoped access" → "Full Dropbox" → give it a name (e.g. "SUNDAY") → click "Create app"',
