/**
 * deep-link.ts — Handler for sunday:// deep links in the Tauri desktop app.
 *
 ...
 *   sunday://research/{session_id}   → open a research session
 *   sunday://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `sunday://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `sunday://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `sunday:` scheme.
 *
 * @example
 * parseDeepLink("sunday://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("sunday://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "sunday:") return null;

    // The URL constructor treats "sunday://research/abc123" such that
