...
export function XRayFooter({ usage, telemetry }: Props) {
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.06)', marginTop: '0.375rem' }}>
      <button
      ...
      >
        <span
          ...
          style={{ background: '#60a5f9', boxShadow: '0 0 6px rgba(96, 165, 249, 0.5)' }}
        />
        <span
          ...
          <div className="grid gap-y-0.5" style={{ gridTemplateColumns: 'auto 1fr', columnGap: '1rem' }}>
            {rows.map((row) => (
              <div key={row.label} className="contents">
                <span className="text-[11px]" style={{ color: '#6b7280', fontFamily: 'monospace' }}>
                  {row.label}
                </span>
                <span
                  className="text-[11px]"
